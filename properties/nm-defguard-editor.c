...
static void
label_set_wrap(GtkWidget *label)
{
#if GTK_CHECK_VERSION(4, 0, 0)
...
static const char *
data_item(NMSettingVpn *setting, const char *key)
{
	const char *value = setting ? nm_setting_vpn_get_data_item(setting, key) : NULL;

	return value ? value : "";
}

static const char *
traffic_label(const char *mode)
{
...
static void
defguard_editor_dispose(GObject *object)
{
...
static void
defguard_editor_class_init(DefguardEditorClass *klass)
{
	G_OBJECT_CLASS(klass)->dispose = defguard_editor_dispose;
}

static void
defguard_editor_init(DefguardEditor *editor)
{
	(void) editor;
}
...
nm_vpn_editor_factory_defguard(NMVpnEditorPlugin *plugin, NMConnection *connection, GError **error)
{
	...
	note = gtk_label_new("Authentication, WireGuard settings, routes, and DNS are managed by Defguard. Run nm-defguard-import to change this profile.");
