...
G_DEFINE_TYPE_EXTENDED(DefguardEditorPlugin, defguard_editor_plugin, G_TYPE_OBJECT, 0,
	G_IMPLEMENT_INTERFACE(NM_TYPE_VPN_EDITOR_PLUGIN, editor_plugin_iface_init))
...
static void
get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *spec)
{
	switch (prop_id) {
	case PROP_NAME:
...
static void
defguard_editor_plugin_init(DefguardEditorPlugin *plugin)
{
	(void) plugin;
}

static EditorFactory
load_editor(GError **error)
{
...
{
	(void) error;
	return NM_VPN_EDITOR_PLUGIN(g_object_new(defguard_editor_plugin_get_type(), NULL));
}
